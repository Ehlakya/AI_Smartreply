...
import { RefreshCw, Paperclip, ChevronLeft, Trash2, Search, ChevronRight, RotateCcw } from 'lucide-react';
...
export default function TrashPage() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ['trash', user?._id, page, debouncedSearch],
    queryFn: () => emailService.getTrash(page, 20, debouncedSearch),
  ...
  const handleBulkAction = async (action, ids = Array.from(selectedEmails)) => {
    ...
    try {
      if (action === 'permanent_delete') {
        ...
        toast.success('Emails deleted permanently');
      } else {
        await emailService.bulkAction(ids, action);
        
        if (action === 'restore') {
          showUndoToast('Emails restored.', async () => {
            await emailService.bulkAction(ids, 'trash');
            queryClient.invalidateQueries(['trash']);
      ...
      if (selectedEmail && ids.includes(selectedEmail._id) && ['restore', 'permanent_delete'].includes(action)) {
    ...
    <div className="flex h-full relative overflow-hidden glass rounded-[2.5rem]">
      ...
      <div className={`min-w-0 w-full h-full flex flex-col transition-all duration-300 ${selectedEmail ? 'md:w-1/3 border-r border-white/10 hidden md:flex' : 'w-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar min-w-0 relative">
          ...
          {emails.length === 0 ? (
            <div className="text-center mt-10 text-foreground/50">Trash is empty.</div>
          ) : (
            emails.map((email) => (
              <motion.div 
                key={email._id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm mb-1 truncate font-medium">{email.subject}</div>
                  ...
                  <div className="mt-3 flex gap-2 justify-between items-center h-6">
                    ...
                    <div className="hidden group-hover:flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                      <button onClick={(e) => { e.stopPropagation(); handleBulkAction('restore', [email._id]); }} className="p-1.5 hover:bg-white/20 rounded-lg transition" title="Restore"><RotateCcw className="w-4 h-4" /></button>
                      <button onClick={(e) => { e.stopPropagation(); handleBulkAction('permanent_delete', [email._id]); }} className="p-1.5 hover:bg-danger/20 text-danger rounded-lg transition" title="Delete Forever"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                </div>
              </motion.div>
...
function TrashDetailView({ email, onBack, onUpdate }) {
  ...
  const handleRestore = async () => {
    setIsProcessing(true);
    try {
      await emailService.restoreFromTrash(email._id);
  ...
  const handlePermanentlyDelete = async () => {
    setIsProcessing(true);
    try {
      await emailService.permanentlyDelete(email._id);
    ...
    <div className="flex flex-col h-full w-full bg-background/50 backdrop-blur-md">
      ...
      <div className="p-4 border-b border-white/10 flex items-center justify-between bg-white/5 shrink-0">
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-xl font-bold truncate">Deleted Message</h2>
          </div>
        </div>
      </div>
          ...
          <>
            ...
            <div className="flex justify-end gap-3 mt-4">
              <button 
                ...
                className="px-6 py-2 rounded-xl bg-primary hover:bg-primary/80 text-white text-sm font-bold shadow-lg transition flex gap-2 items-center disabled:opacity-50"
              >
              ...
              <button 
                ...
                disabled={isProcessing}
                className="px-6 py-2 rounded-xl bg-danger hover:bg-danger/80 text-white text-sm font-bold shadow-lg shadow-danger/20 transition flex gap-2 items-center disabled:opacity-50"
              >
